...
grep -q 'Optional pattern discovery' <<<"$html"
grep -q 'structure you cannot' <<<"$html"
grep -q 'long-horizon profile' <<<"$html"
grep -q 'patterns, coverage, and vault memory' <<<"$html"
! grep -q 'changed against your own baseline' <<<"$html"
