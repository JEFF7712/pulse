...
class DiscoveryEngine:
    def __init__(self, database_path, vault_root, llm, notification_channel=None):
    ...
    async def run_discovery(self, cadence: str, target_date: date) -> DiscoveryResult:
        ...
        async with connect_db(self._db_path) as db:
            ...
            # Step 2: Aggregate stats for each day in window
            current = start_date
            while current <= target_date:
                await analytics.aggregate_day(current.isoformat())
                current += timedelta(days=1)

            # Step 3: Gather stats and events
            # end is exclusive — day after target_date
            ...
            # Step 4: Get baselines for weeks 1-4 back from target_date
            baselines: list[dict] = []
        ...
        baselines_text = routines_text  # routines serve as baseline context

        # Step 7: Build prompt, call LLM, parse response
        prompt_dict = build_discovery_prompt(
            cadence=cadence,
            date_range=date_range,
            event_summary=event_summary or "(no events in window)",
