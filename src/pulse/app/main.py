...
def create_app(
    ...
    async def telegram_webhook(
        ...
        # A reply may quote the original notification rather than repeat its text, so
        # look in both places for the `Context:` marker that identifies the proposal.
...
def _register_push_route(app: FastAPI, push_conn, settings_dependency) -> None:
    ...
    source_name = push_conn.get_source_name()
    ...
    async def handler(
        ...
        connector_config = s.connectors.get(source_name)
