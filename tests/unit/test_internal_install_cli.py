...
from pulse.app.commands.onboard import internal_install


def test_internal_install_ready_prints_success(capsys):
    internal_install(SimpleNamespace(phase="ready"))
...
def test_internal_install_noninteractive_prints_onboard_hint(capsys):
    internal_install(SimpleNamespace(phase="noninteractive"))
