...
def test_digest_action_writes_digest_for_seeded_event(tmp_path, monkeypatch) -> None:
    target_day = date(2026, 3, 22)
    event_title = "Runtime E2E seeded event"
    ...
    digest_path = vault_path / "01-Daily" / f"{target_day.isoformat()}.md"
    assert digest_path.exists()
    assert event_title in digest_path.read_text(encoding="utf-8")
