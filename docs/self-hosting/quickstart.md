...
Use **`pulse auth`** to see what is authorized and what still needs credentials, and **`pulse auth <source>`** (`google`, `github`, `spotify`, `plaid`, `oura`; `gmail`/`calendar`/`youtube` all resolve to `google`) to run one flow directly. It checks prerequisites and prints the exact redirect URI to register *before* opening a browser. The same flows also run from **Configure → Connectors** (loopback callbacks on `8888`, `8891`, `8893`, `8894` as applicable; Spotify's redirect URI must be registered as `http://127.0.0.1:8888/callback`, since Spotify rejects the `localhost` hostname as insecure). Complete this **before** `pulse init` if the first pull should hit those APIs. Browser history skips browser OAuth here.
...
| `pulse auth [source]` | Authorization status, or run one OAuth/Link flow |
| `pulse review` | Force a discovery pass now (if configured) |
| `pulse test-telegram` | One-off Telegram test |

Re-open **Configure → Connectors** anytime to re-auth or edit `pulse.toml`.

### Pattern discovery (optional) {#pattern-discovery-optional}
