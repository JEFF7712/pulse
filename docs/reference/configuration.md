...
**Facts can be corrected, with your say-so.** Plain facts with a truth value the data can check (location, school, employer) live in `04-Config/facts.md`. The agent proposes a correction with evidence; Pulse notifies you and applies it only when you reply `yes`. Nothing changes silently, and the stated profile is never rewritten either way. Replies arrive over `POST /webhooks/telegram`, which needs to be reachable from Telegram — see the runbook.
