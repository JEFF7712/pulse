...
**Facts can be corrected, with your say-so.** Plain facts with a truth value the data can check (location, school, employer) live in `04-Config/facts.md`. The agent proposes a correction with evidence; Pulse notifies you and applies it only when you reply `yes`. Nothing changes silently, and the stated profile is never rewritten either way. Replies arrive over Telegram long-polling, which `pulse run` starts automatically when `telegram_bot_token` and `telegram_chat_id` are set — no public endpoint, tunnel or port-forward needed. If you would rather push, register a webhook at `/webhooks/telegram` and polling defers to it. See the runbook.
